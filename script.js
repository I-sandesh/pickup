...
const getRandomTime = (level) => {
    return Math.floor(Math.random() * 1000) + 1000 - (level * 200);
}
...
const handlePlateClick = (event) => {
    ...
    if(plate.classList.contains('color')){
        score += parseInt(plate.dataset.score);
...
const startGame = () => {
    gameOver = false;
    GAME_TIME = 30000;
    document.querySelector('.store').classList.remove('hide');
